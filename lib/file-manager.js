...
class FileManager {
  ...
  writeFile(sessionId, filename, content, encoding = 'utf8') {
    ...
    // Validate path safety BEFORE extension check to catch traversal attempts early
    const filePath = this._resolveSafe(sessionId, filename);
  ...
  readFile(sessionId, filename, encoding = 'base64') {
    ...
    // Validate path safety BEFORE extension check
