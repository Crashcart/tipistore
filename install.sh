#!/bin/bash
command -v docker &>/dev/null || { echo "❌ Docker required"; exit 1; }; command -v node &>/dev/null || { echo "❌ Node.js required"; exit 1; }; [ -f .env ] || { AUTH_SECRET=$(node -e "console.log(require('crypto').randomUUID())"); ADMIN_PASSWORD=$(node -e "console.log(require('crypto').randomBytes(8).toString('hex'))"); cat > .env << EOF
NODE_ENV=production
PORT=31337
